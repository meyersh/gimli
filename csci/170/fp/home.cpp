...
string
listUsers(bool newline = false) /* setting this to true will 
				   return all users \n separated. */
{
  stringstream output;
  ifstream usersfile;
...
main()
{
  ...
  if ( request.META["REQUEST_METHOD"] == "POST" )
	...
	{
	  output << "error=Invalid session key. Please " << urlEncode(" [ <a href=\"login.cgi\">login</a> ]")<< "\n";
	   ...
	   )
	{
	  ...
	  if ( *action == "loadprofile" )
		...
		output << "whiteboard=" << username << " is PREVENTING you from viewing his or her whiteboard.\n";
	      else
	   ...
	   */
	  ...
	  else if ( *action == "writePublicMessage" )
	    {
	      ...
	      msgs ms(request.POST["toUsername"]);
	      ms.insert( m );
	      ms.writeMessages();
	      ...
	      for (int i=0; i < ms.size(); i++)
		{
		  /* if ( !ms.hasMessage(i) )
		   ...
		   if ( ms[i].inResponseTo == 0 && 
		       (ms[i].sender == loggedOnUser || username == loggedOnUser) 
		       ) 
	   ...
	   */
	  ...
	  else if ( *action == "banUsers" )
	    {
	      ...
	      if ( request.hasPOST("bannedusers") && request.POST["bannedusers"] != "1" )
		{
		  ...
		  output << "error=Settings saved. " << bl.size() << " users banned." << endl;
		}
   ...
   */
    ...
    {
      cout << "location:/~meyersh/fp/profile.html\n\n";
