...
CREATED     : 17 Mar 2011
DESCRIPTION : Creates a binary tree to play a (learning) game of 20 question
              meeting the following criteria:
 
  1. Initial question (when tree is empty): What is it?
  2. Question at an internal node: Does it have a/an ____? (Item in blank comes 
     from node.)
  3. Question at a leaf node: Is it a/an ____? (Item in blank comes from node.)

  Questions if not found at leaf:
  1. What is it?
  2. What property does it have that makes it different from a/an ____? 
     (Item in blank comes from item in leaf node being expanded.)

  BONUS: There will be 5 bonus points for any project that meets the basic 
         requirements and also allows trees to be save to files and read in 
         from files.
   ...
   DESCRIPTION: Return true if the character is a vowel. */
{
   ...
   for (char v = 0; vowels[v] != NULL; v++)
      if (vowels[v] == toupper(inpt))
	 return true;
   return false;
...
/* PARAMS     : Node pointer
   RETURNS    : string
   DESCRIPTION: Given a node pointer, returns a `|'-separated representation 
   of the node's data suitable for printing or saving. For simplicity and
   garanteed uniqueness, the pointer addresses are used as the NODE_IDs. */
{
   if (!root)
      return "";
   ...
   DESCRIPTION: Opens a file, saving the output of print_binary_tree() to it. */
{
   ...
   save_file << "# This file is automatically generated. "
	     << "All lines beginning with `#' are \n"
	     << "# ignored. Each field is separated by a `|' symbol. Any unique"
	     << " identifier \n# (except 0, which is NULL.) will work for a "
	     << "node.\n"
	     << "# NODE_ID | LEFT_CHILD_ID | RIGHT_CHILD_ID | NODE_DATA\n"
	     << print_binary_tree(root) << endl;
...
/* PARAMS     : string (line from data file)
   RETURNS    : Node ptr
   DESCRIPTION: Given a line from the data file, create a node and populate
   it's data field. Returns the pointer to this new node. */
{
...
int main(int argc, char **argv)
{
   ...
   if (args.size() > 1) // try to load the specified file.
	 ...
	 cout << "Error loading file '" << args[1] << "'\n";
      ...
      /* We always remember the filename, even if it's not a legit file. 
	 this is used later if SAVE or LOAD is used w/o a filename. */ 
      filename = args[1];
      }
   ...
   while (std::getline(cin, cmd))
	 ...
	 case IS_IT_A:
	    ...
	    break;
	 case WHAT_PROP:
	    prompt = "What property distinguishes it from a" 
	       + string((isvowel(curnode->left->data[0]) ? "n " : " "))
	       + curnode->left->data + "?> ";
	    break;
	 default:
	    prompt = "> ";
